package edu.my.rstetsenko.zenua.service;
...
//            sendIntent.putExtra(Constants.EXTRA_SOURCE, intent.getStringArrayExtra(Constants.EXTRA_SOURCE));
...
//    }
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent == null || !intent.hasExtra(Constants.EXTRA_SOURCE)) {
            return;
        }
        ...
        try {
            URL url = new URL(intent.getStringArrayExtra(Constants.EXTRA_SOURCE)[0]);
    ...
    private void addRateRow(int sourceId, long updateDate, double usd, double eur, double rub) {
        ...
        Uri resultUri = getContentResolver().insert(RateContract.RateEntry.CONTENT_URI, values);
    ...
    private void addDoubleRateRow(int sourceId, long updateDate, double usdBuy, double usdSell,
        ...
        Uri resultUri = getContentResolver().insert(RateContract.DoubleRateEntry.CONTENT_URI, values);
        long id = ContentUris.parseId(resultUri);
